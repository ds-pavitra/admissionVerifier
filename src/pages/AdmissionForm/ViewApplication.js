...
const ViewApplication = ({ application, fetchlist, toggle, verificationStatus, verifyType }) => {
    ...
    const fetchData = async () => {
        try {
            const response = await apiRequestAsync("get", `${apiBaseUrl}/applicant/${application}`, null);
    ...
    const handleSubmit = async () => {
        if ((status === "rejected") && reason.trim() === "") {
            setError("Reason is required for rejection status.");
            return;
        }
        ...
        const payload = {
            ...
            remark: status === "approved" ? "" : reason, // If approved, no reason needed
        };

        try {
            const response = await apiRequestAsync("post", `${apiBaseUrl}/applicant/${application}/${verifyType}`, payload);
            if (response.status === 200) {
                fetchlist();
                toggle();
    ...
    return (
        <div className="container">
            ...
            {/* Reason Input (Only required for pending/rejected) */}
            {(status === "rejected") && (
